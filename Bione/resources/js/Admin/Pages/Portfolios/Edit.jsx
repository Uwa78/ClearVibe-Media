...
import PortfolioDetails from "@/Admin/Components/SectionCustomize/PortfolioDetails"
...
export default function Edit() {
    ...
    const { lang, default_lang, sections, portfolio } = usePage().props
    const dispatch = useDispatch()
    const dragItem = useRef(null)
    const dragOverItem = useRef(null)
    const [isExpandInfo, setIsExpandInfo] = useState(false)
    const [isOpen, setIsOpen] = useState(true)
    const [expandCustomize, setExpandCustomize] = useState("PortfolioDetailsPage")
    ...
    const handleUpdate = () => {
        ...
        if (!errors) {
            router.put(route("admin.portfolios.update", portfolio), {
                pageData,
                pageInfo,
                customizeSections
            })
        ...
        function validateLanguageTitles(data) {
            for (const langCode in data) {
                ...
                } else if (langData.thumbnail_image === "") {
                    return `The ${languageName} thumbnail image is required.`
                } else if (langData.sub_title === "") {
                    return `The ${languageName} subtitle is required.`
                } else if (langData.sub_title?.length > 255) {
                    return `The ${languageName} subtitle must not exceed 255 characters.`
                } else if (langData.details === "") {
                    return `The ${languageName} details is required.`
    ...
    switch (expandCustomize) {
        case "PortfolioDetailsPage":
            customizeSection = <PortfolioDetails />
            break
        case "Hero":
    ...
    return (
        <PageCustomizeLayout type="portfolios">
            ...
            <div className="customize-header-actions">
                <Link href={route("admin.portfolios.index")} className="dismiss">
            ...
            <div className="customize-section-wrap">
                ...
                <div className="customize-section-area">
                    ...
                    <div className="customize-sections">
                        <div className="customize-sections-item" onClick={() => handleExpand("PortfolioDetailsPage")}>
                            <div className="customize-sections-item-title">
                                <h3>Portfolio Details Page</h3>
                            </div>
